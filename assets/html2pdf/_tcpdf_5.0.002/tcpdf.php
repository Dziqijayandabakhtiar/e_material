...
if (!class_exists('TCPDF', false)) {
	...
	*/
	class TCPDF {
		 ...
		 * @access public
		 */
		public function __construct($orientation='P', $nama_staff='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false) {
		 ...
		 */
		public function setPageUnit($nama_staff) {
			$nama_staff = strtolower($nama_staff);
			//Set scale factor
			switch ($nama_staff) {
				// points
				case 'px':
				...
				// unsupported unit
				default : {
					$this->Error('Incorrect unit: '.$nama_staff);
					break;
				}
			}
			$this->pdfunit = $nama_staff;
